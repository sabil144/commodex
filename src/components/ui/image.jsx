 ...
 *
 * Two local deviations from the Base44 original:
 *  - `fallbackSrc` overrides FALLBACK_IMAGE_URL, so a self-hosted asset can
 *    stand in when the CDN is unreachable (blocked host, offline, strict CSP).
 *  - the plain-<img> branches apply the same object-fit as ResponsiveImage,
 *    which otherwise only holds on the optimized path.
 */
const Image = React.forwardRef(
  (
    ...
    const fallbackUrl = fallbackSrc || FALLBACK_IMAGE_URL

    const imageProps = {
      ...props,
      onError: handleError,
      className: cn(fittingType === "fit" ? "object-contain" : "object-cover", props.className),
    }
    ...
    if (!src) {
      ...
      return <img ref={ref} src={fallbackUrl} {...imageProps} data-empty-image />
    }
    ...
    if (!parsed) {
      ...
      const imageSrc = isErrorMode ? fallbackUrl : getOriginalImageUrl(src, parsedSource)
      return (
